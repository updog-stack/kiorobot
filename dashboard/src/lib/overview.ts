...
// ===== 총 매출 (장비 + 라이선스 + 기타, 단위: 원) =====
// 2025년 구분별 월별(시트 실데이터). 2026년은 노션 라이브(/api/sales-monthly)로 채움.
export const SALES_CATS = ["장비", "라이선스", "기타"] as const;
export type SalesCat = (typeof SALES_CATS)[number];

export const SALES_2025: Record<SalesCat, number[]> = {
  장비: [
    10_582_000, 26_725_600, 59_736_600, 26_177_800, 58_377_000, 21_076_000,
    25_564_000, 58_810_000, 38_987_000, 21_285_000, 41_734_000, 8_030_000,
  ],
  라이선스: [
    1_815_000, 1_980_000, 2_849_000, 3_795_000, 1_881_000, 2_706_000,
    3_795_000, 1_826_000, 1_265_000, 2_222_000, 2_871_000, 3_036_000,
  ],
  기타: [
    2_264_128, 135_000, 1_609_704, 1_021_556, 1_985_000, 2_632_000,
    1_981_000, 712_300, 2_165_000, 3_497_500, 1_423_600, 1_161_000,
  ],
};

// 선택된 구분들의 월별 합(길이 len)
export function sumCats(
  byCat: Partial<Record<string, number[]>>,
  cats: readonly string[],
  len: number
): number[] {
  return Array.from({ length: len }, (_, i) =>
    cats.reduce((s, c) => s + (byCat[c]?.[i] ?? 0), 0)
  );
}

// 총 매출 기본 시리즈(작년=3분류 합). cur 은 전체현황에서 노션 라이브로 덮어씀.
// cur 정적값 = 2026 월별(장비 실데이터, 라이선스/기타는 노션 미동기화로 0) — 경영지표 등 폴백용.
export const totalSales: Mseries = {
  key: "total",
  label: "총 매출",
  unit: "won",
  cur: [
    12_408_000, 37_070_000, 40_645_000, 34_606_000, 21_901_000, 15_136_000,
    17_512_000,
  ],
  prev: sumCats(SALES_2025, SALES_CATS, 12),
};

// (하위호환) 이전 '장비 매출' 이름 참조 방지용 — 총매출로 대체됨
export const equipment: Mseries = totalSales;
...
export const kicc: Mseries = {
  ...
  label: "KICC",
  unit: "count",
  cur: [17_204, 15_582, 16_183, 17_973, 14_465, 15_089],
  prev: [
