...
// 마케팅(당근·네이버 블로그)은 로그인된 브라우저 세션이 필요해 로컬 PC(Windows)에서만 수집 가능.
//   '데이터 동기화'(마케팅 화면)를 누르면 아래 스크립트를 각각 새 콘솔 창에서 실행.
...
function launchMarketingBats() {
  const launched = [];
  for (const j of MARKETING_JOBS) {
    if (!existsSync(join(ROOT_DIR, j.script))) continue;
    // 콘솔 창 없이 백그라운드 실행(windowsHide) — node 직접 실행, 로그는 파일로.
    //   ※ 당근·네이버는 헤드리스 차단이라 '브라우저 창'은 뜸(네이버 잠깐, 당근 상주). cmd 콘솔은 안 뜸.
    const logFd = openSync(join(__dirname, "data", `_mkt-${j.title}.log`), "a");
    const child = spawn(process.execPath, [j.script], {
      cwd: ROOT_DIR,
      detached: true,
