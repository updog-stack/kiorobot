...
// 마케팅(당근·네이버 블로그)은 로그인된 브라우저 세션이 필요하다.
//   · 로컬(Windows): 브라우저 프로필에 세션이 있으므로 그대로 실행. 창이 잠깐 뜬다.
//   · 서버(리눅스) : 이식된 storageState(server/data/*-state.json)로 로그인하고,
//                    헤드리스가 차단되므로 Xvfb 가상화면 위에서 실행한다.
//     서버도 cron 으로 주기 수집하지만(네이버 09·18시 / 당근 매시), 버튼을 누르면 즉시 1회 돈다.
//   ※ .bat(한글 파일명)은 spawn 시 인코딩 문제로 못 찾으므로, ASCII 경로의 node 스크립트를 직접 실행.
const ROOT_DIR = join(__dirname, ".."); // dashboard 루트(server/.. )
const MARKETING_JOBS = [
  // data: 마지막 수집 시각(updatedAt)을 읽어 '얼마나 묵었나'를 판단할 파일
  { title: "Naver_Blog", script: "server/naver-blog-scraper.mjs", data: "naver-blog.json" }, // 1회 수집 후 종료
  { title: "Danggeun_Ads", script: "server/daangn-ads-daemon.mjs", data: "daangn-ads.json" }, // 상주(즉시 1회 + 30분마다)
];
// 서버는 상주 데몬 대신 1회성 스크립트를 쓴다(cron 과 동일한 것).
const MARKETING_JOBS_SERVER = [
  { title: "Naver_Blog", script: "server/naver-blog-scraper.mjs", data: "naver-blog.json" },
  { title: "Danggeun_Ads", script: "server/daangn-ads-once.mjs", data: "daangn-ads.json" },
];
const marketingJobs = () => (process.platform === "win32" ? MARKETING_JOBS : MARKETING_JOBS_SERVER);

// 같은 수집기를 겹쳐 띄우지 않도록 실행 중 표시(브라우저를 띄우므로 중복 실행이 특히 나쁨)
const marketingRunning = new Set();

function launchMarketingJob(j) {
  if (!existsSync(join(ROOT_DIR, j.script))) return false;
  if (marketingRunning.has(j.title)) return false;
  const onServer = process.platform !== "win32";
  // 콘솔 창 없이 백그라운드 실행(windowsHide) — 로그는 파일로.
  //   ※ 당근·네이버는 헤드리스 차단이라 '브라우저 창'은 뜸(로컬: 실제 창 / 서버: Xvfb 가상화면).
  const logFd = openSync(join(__dirname, "data", `_mkt-${j.title}.log`), "a");
  // 서버는 xvfb-run 으로 감싸 가상 디스플레이를 붙인다.
  const [cmd, args] = onServer
    ? ["xvfb-run", ["-a", process.execPath, j.script]]
    : [process.execPath, [j.script]];
  ...
  marketingRunning.add(j.title);
  const done = () => marketingRunning.delete(j.title);
  child.on("error", (e) => { console.error(`마케팅 수집 실행 실패(${j.script}):`, e.message); done(); });
  child.on("exit", done);
  // 상주 데몬(로컬)은 exit 가 안 오므로 넉넉한 시간 뒤 자물쇠를 푼다.
  setTimeout(done, 10 * 60 * 1000).unref?.();
  child.unref();
  closeSync(logFd); // 자식이 상속한 뒤 부모 핸들은 닫음
  return true;
}

function launchMarketingBats() {
  return marketingJobs().filter(launchMarketingJob).map((j) => j.script);
}

// ── 마케팅 자동 동기화 — '마지막 수집 시각 기준' 1시간 ──────────────────
// cron(네이버 09·18시 / 당근 매시)과 별개로, 데이터가 1시간 넘게 묵으면 채운다.
//   · 수동 '데이터 동기화'를 누르면 그 시점부터 다시 1시간을 센다(파일 시각 기준이라 자동으로 그렇게 됨)
//   · cron 이 방금 돌았으면 데이터가 신선하므로 그냥 건너뛴다 → 서로 중복되지 않는다
//   · BFF 가 재시작돼도 파일 시각만 보므로 상태를 잃지 않는다
const MARKETING_MAX_AGE_MS = 60 * 60 * 1000; // 1시간
const MARKETING_CHECK_MS = 5 * 60 * 1000; // 5분마다 점검

async function marketingAgeMs(dataFile) {
  const j = await readJson(join(__dirname, "data", dataFile));
  const t = j?.updatedAt ? Date.parse(j.updatedAt) : NaN;
  return Number.isFinite(t) ? Date.now() - t : Infinity; // 파일이 없거나 시각이 없으면 '아주 묵음'
}

async function refreshStaleMarketing() {
  for (const j of marketingJobs()) {
    try {
      const age = await marketingAgeMs(j.data);
      if (age < MARKETING_MAX_AGE_MS) continue;
      if (launchMarketingJob(j)) {
        const mins = age === Infinity ? "수집 기록 없음" : `${Math.round(age / 60000)}분 경과`;
        console.log(`[마케팅 자동동기화] ${j.title} 재수집 (${mins})`);
      }
    } catch (e) {
      console.error(`[마케팅 자동동기화] ${j.title} 확인 실패:`, e.message);
    }
  }
}

function scheduleMarketingRefresh() {
  setTimeout(refreshStaleMarketing, 60 * 1000); // 기동 1분 뒤 1회
...
app.post("/api/collect", (req, res) => {
  ...
  //   서버는 이식된 세션 + Xvfb 로 직접 수집하므로 로컬 PC가 꺼져 있어도 된다.
  //   (예전에는 서버가 로컬 수집기에 요청을 넘겼다 — 로컬이 꺼져 있으면 아무 일도 안 일어났음)
  if (scope === "marketing") {
    const bats = launchMarketingBats();
    marketingTrigger = { requestedAt: Date.now() }; // 로컬 데몬이 떠 있으면 그쪽도 갱신
    return res.json({
      started: true,
      marketing: true,
      bats,
      note: bats.length ? "수집 중 · 1분쯤 뒤 새로고침" : "수집 스크립트를 찾지 못했습니다.",
    });
  }
