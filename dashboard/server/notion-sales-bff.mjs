...
const MARKETING_JOBS = [
  { title: "Naver_Blog", script: "server/naver-blog-scraper.mjs" }, // 1회 수집 후 종료
  { title: "Danggeun_Ads", script: "server/daangn-ads-daemon.mjs" }, // 상주(즉시 1회 + 30분마다)
];
// 서버는 상주 데몬 대신 1회성 스크립트를 쓴다(cron 과 동일한 것).
const MARKETING_JOBS_SERVER = [
  { title: "Naver_Blog", script: "server/naver-blog-scraper.mjs" },
  { title: "Danggeun_Ads", script: "server/daangn-ads-once.mjs" },
];
function launchMarketingBats() {
  const onServer = process.platform !== "win32";
  const jobs = onServer ? MARKETING_JOBS_SERVER : MARKETING_JOBS;
  const launched = [];
  for (const j of jobs) {
    if (!existsSync(join(ROOT_DIR, j.script))) continue;
    ...
    // 서버는 xvfb-run 으로 감싸 가상 디스플레이를 붙인다. 없으면 그냥 실행(실패는 로그로).
    ...
    child.on("error", (e) => console.error(`마케팅 수집 실행 실패(${j.script}):`, e.message));
    child.unref();
    closeSync(logFd); // 자식이 상속한 뒤 부모 핸들은 닫음
    launched.push(j.script);
  }
  return launched;
}
