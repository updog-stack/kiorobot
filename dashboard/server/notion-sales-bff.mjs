...
//   '데이터 동기화'(마케팅 화면)를 누르면 아래 .bat을 새 창으로 실행 = 더블클릭과 동일.
const MARKETING_BATS = ["네이버블로그수집.bat", "당근광고수집.bat"];
const ROOT_DIR = join(__dirname, ".."); // dashboard 루트(.bat 위치)
function launchMarketingBats() {
  const launched = [];
  for (const bat of MARKETING_BATS) {
    if (!existsSync(join(ROOT_DIR, bat))) continue;
    // 새 콘솔 창에서 실행(더블클릭과 동일). detached + unref → BFF와 독립.
    const child = spawn("cmd.exe", ["/c", "start", "", "cmd", "/c", bat], { cwd: ROOT_DIR, detached: true, stdio: "ignore", windowsHide: false });
    child.unref();
    launched.push(bat);
  }
  return launched;
