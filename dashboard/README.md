...
| **경영 지표** | 전체현황 분석 요약 — 월별 추이·작년 **동월** 대비 평가·**연말 예상(작년 계절성 반영)**·**작년=100점 성과 점수 + 원포인트 처방** | overview 데이터셋 |
| **거래(TR) 현황** | VAN사별(코밴·다우데이타·**KICC**) 월별 거래 건수 + 합산 · **CMS 매출** 섹션 | 코밴 CATECA, 다우데이타 DDWM, KICC·CMS(구글시트 참고값) |
| **일정** | **구글캘린더 월간 캘린더** — 월 이동·일정 클릭 상세·**생성/수정/삭제 양방향 동기화(45초 폴링)**·다중 캘린더(설치·방문·연차·당직) | 구글 캘린더(iCal · 서비스계정 API) |
...
> 업무 / 할 일 탭은 아직 자리(placeholder)만 잡혀 있습니다.
