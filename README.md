...
> 현재 버전: **v1.30** (버전 규칙: 대시보드 메뉴 1개당 버전 1 — 메뉴 11개. v1.20·v1.30은 새 메뉴 없이 기존 기능을 고도화한 기능 릴리스)
...
| 메뉴 | 구현 내용 | 데이터 출처 |
|------|-----------|-------------|
| **전체 현황** | 핵심 운영지표 한눈 요약 — 장비·CMS 매출, VAN 건수, CS, 신규/폐업 사업자 (월별·작년 대비) · **유튜브 채널지표 카드**(구독자·조회수·최근영상) 🆕 · **데이터 동기화 버튼 + 매일 8시 자동수집** 🆕 | 구글시트 예시값 + 유튜브 Data API + VAN 자동수집 |
...
| **거래(TR) 현황** | VAN사별(코밴 · 다우데이타 · **KICC**) 월별 거래 건수 + 합산 · **월별 결제 추이 그래프(2025년~, 건수+다우 결제금액, 년도 필터)** 🆕 · **CMS 매출** 섹션 | 코밴·다우데이타 자동수집(다우 금액 포함), KICC·CMS(구글시트 참고값) |
...
> 버전별 상세 문서: **[v1.30](docs/v1.30.md)** · [v1.20](docs/v1.20.md) · [v1.11](docs/v1.11.md) · [v1.10](docs/v1.10.md) · [v1.3](docs/v1.3.md) · [v1.2](docs/v1.2.md) · [v1.0](docs/v1.0.md) · [v0.1](docs/v0.1.md) (각 버전의 기능·파일별 변경 내역)
>
> 버전 규칙: **대시보드 메뉴 1개당 버전 1 증가**(메뉴 11개 → v1.11). v1.20·v1.30은 새 메뉴 없이 기존 기능을 대폭 고도화한 **기능 릴리스**.
